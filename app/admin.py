...
from app.viewTodo import TodoChartview
from lib.GetJenkinsAPI import get_jobs_data
from app.models import Money, Jenkins, Todo, TodoChart,  SiteMap
from app.views import tesControlview, testchart1view, testchartBySqlView, siteMapview

'''
Devops TODO 系统
'''
class CheckedListFilter(admin.SimpleListFilter):
    title ='完成'
...
class TodoAdmin(admin.ModelAdmin):
    ...
    # 增加自定义按钮
    actions = ['finished_button']

    def finished_button(self, request, queryset):
    ...
    finished_button.short_description = 'Finished'
    finished_button.icon = 'el-icon-s-promotion'



# @admin.register(Pior)
class PiorAdmin(admin.ModelAdmin):
    list_display = ('id','name')

# @admin.register(Plantime)
...
@admin.register(TodoChart)
class TodoChartAdmin(admin.ModelAdmin):
    def changelist_view(self, request, extra_context=None):
        return TodoChartview(request)
